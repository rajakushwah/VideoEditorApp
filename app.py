...
# @app.route('/merge', methods=['POST'],endpoint='merge_videos_endpoint')
# def merge_videos():
#     try:
...
#
#         # Close clips after processing
#         for clip in clips:
#             clip.close()
#         final_clip.close()
#
#         # Generate a unique ID and expiry time for the merged video
#         unique_id = str(uuid.uuid4())
#         expiry_time = datetime.now() + timedelta(hours=24)  # Assume expiry time is 24 hours
#
#         # Check if the filename already exists in the database
#         conn = sqlite3.connect(app.config['DATABASE'])
#         cursor = conn.cursor()
#
#         cursor.execute("SELECT 1 FROM videos WHERE filename = ?", (merged_filename,))
#         if cursor.fetchone():
#             # If the filename already exists, generate a new one and retry
#             new_choice = random.randint(5, 100)
#             merged_filename = f"merged_video_{new_choice}.mp4"
#             merged_filepath = os.path.join(app.config['UPLOAD_FOLDER'], merged_filename)
#             # Check again for uniqueness
#             cursor.execute("SELECT 1 FROM videos WHERE filename = ?", (merged_filename,))
#             if cursor.fetchone():
#                 # If still a duplicate, return error
#                 return jsonify({'error': f'File "{merged_filename}" still exists in database'}), 409
#
#         try:
